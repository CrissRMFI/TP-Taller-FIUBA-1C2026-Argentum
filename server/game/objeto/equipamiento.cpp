...
std::vector<uint16_t> Equipamiento::equiparArma(uint16_t idItem) {
    ...
    if (arma != 0)
        desplazados.push_back(arma);
    if (baculo != 0)
        desplazados.push_back(baculo);
    arma = idItem;
    baculo = 0;
    return desplazados;
}
...
uint16_t Equipamiento::getArma() const {
    return arma;
}
uint16_t Equipamiento::getBaculo() const {
    return baculo;
}
uint16_t Equipamiento::getDefensa() const {
    return defensa;
}
uint16_t Equipamiento::getCasco() const {
    return casco;
}
uint16_t Equipamiento::getEscudo() const {
    return escudo;
}
