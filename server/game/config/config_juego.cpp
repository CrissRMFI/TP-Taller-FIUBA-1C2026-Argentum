...
float ConfigJuego::factorVidaClase(ClasePersonaje clase) const {
    switch (clase) {
        case ClasePersonaje::GUERRERO:
            return fVidaGuerrero;
        case ClasePersonaje::PALADIN:
            return fVidaPaladin;
        case ClasePersonaje::CLERIGO:
            return fVidaClerigo;
...
// El guerrero no puede meditar: devuelve 0 para que cualquier cálculo que lo llame por error
// produzca recuperación nula en lugar de comportamiento indefinido.
float ConfigJuego::factorMeditacionClase(ClasePersonaje clase) const {
    switch (clase) {
        case ClasePersonaje::GUERRERO:
            return 0.0f;
        case ClasePersonaje::PALADIN:
            return fMeditacionPaladin;
        case ClasePersonaje::CLERIGO:
            return fMeditacionClerigo;
        case ClasePersonaje::MAGO:
            return fMeditacionMago;
    }
    return 0.0f;
}
