...
public:
    ...
    // La posicion inicial la decide el dominio leyendo cfg.spawnInicial; la capa de red no la
    // dicta. Eso mantiene a Server/Aceptador/Cliente agnosticos del concepto de Posicion.
    std::list<EventoSalida> conectarJugador(uint16_t id, const std::string& nombre,
                                            ClasePersonaje clase, Raza raza, uint16_t cabeza,
                                            uint16_t cuerpo);
    std::list<EventoSalida> desconectarJugador(uint16_t id);

    std::list<EventoSalida> ejecutarComando(const uint16_t idCliente,
...
private:
    ...
    EventoSalida armarPosicionNpcPara(uint16_t idCliente, uint16_t idNpc, const Posicion& posicion,
                                      uint16_t cuerpo);
    ...
    bool agregarItemEnSueloCercano(const Posicion& origen, uint16_t idItem,
    ...
    std::list<EventoSalida> ejecutarEmpezarMover(uint16_t idCliente,
                                                 const ComandoEmpezarMover& comando);
    ...
    std::list<EventoSalida> ejecutarDesequipar(uint16_t idCliente,
                                               const ComandoDesequipar& comando);
    ...
    std::list<EventoSalida> ejecutarDepositarItem(uint16_t idCliente,
                                                  const ComandoDepositarItem& comando);
    std::list<EventoSalida> ejecutarDepositarOro(uint16_t idCliente,
                                                 const ComandoDepositarOro& comando);
    std::list<EventoSalida> ejecutarRetirarItem(uint16_t idCliente,
                                                const ComandoRetirarItem& comando);
    std::list<EventoSalida> ejecutarRetirarOro(uint16_t idCliente,
                                               const ComandoRetirarOro& comando);
    ...
    std::list<EventoSalida> ejecutarChatGlobal(uint16_t idCliente,
                                               const ComandoChatGlobal& comando);
    std::list<EventoSalida> ejecutarChatPrivado(uint16_t idCliente,
    ...
    std::optional<uint16_t> buscarIdJugadorEn(
            const Posicion& posicion, std::optional<uint16_t> idExcluido = std::nullopt) const;
    std::optional<Posicion> buscarPosicionLibreCercaDe(
            const Posicion& origen, std::optional<uint16_t> idJugadorExcluido = std::nullopt) const;
    std::list<EventoSalida> actualizarCriaturas();

    std::optional<Jugador> buscarJugadorCercano(const Criatura& criatura) const;
    std::vector<Posicion> calcularDestinosHacia(const Posicion& origen,
    ...
    std::list<EventoSalida> ejecutarAtaqueAJugador(uint16_t idCliente, Jugador& atacante,
                                                   const ComandoAtacar& comando);
    std::list<EventoSalida> ejecutarAtaqueACriatura(uint16_t idCliente, Jugador& atacante,
                                                    Criatura& criatura);
    ...
    bool dropearOroEnSueloCercano(const Posicion& origen, uint32_t cantidad,
                                  Posicion& posicionFinal);
    std::list<EventoSalida> procesarDropsJugadorMuerto(Jugador& jugador,
                                                       const Posicion& posicionMuerte);

    // Centraliza el broadcast de muerte de un jugador: notifica a los demás jugadores del mismo
    // mapa, procesa los drops y re-emite la posición con el nuevo estado de fantasma.
    std::list<EventoSalida> emitirMuerteJugador(Jugador& victima, const Posicion& posicionMuerte);

    // Helpers de validacion NPC-por-id. Devuelven nullptr si el id no matchea, si el NPC esta en
    // otro mapa o si esta fuera del rango de interaccion.
