...
uint16_t ReglasJuego::calcularVidaMaxima(const ConfigJuego& cfg, Raza raza, ClasePersonaje clase,
...
uint16_t ReglasJuego::calcularManaMaximo(const ConfigJuego& cfg, Raza raza, ClasePersonaje clase,
...
uint32_t ReglasJuego::calcularPerdidaExperienciaMuerte(const ConfigJuego& cfg,
...
uint32_t ReglasJuego::calcularExperienciaImpacto(const ConfigJuego& cfg, uint16_t danioAplicado,
                                                 uint8_t nivelAtacante, uint8_t nivelObjetivo) {
    const int diff =
            static_cast<int>(nivelObjetivo) - static_cast<int>(nivelAtacante) + cfg.expBonusNivel;
...
uint32_t ReglasJuego::calcularExperienciaKill(const ConfigJuego& cfg, uint16_t vidaMaxObjetivo,
                                              uint8_t nivelAtacante, uint8_t nivelObjetivo,
...
uint32_t ReglasJuego::calcularDropOroNpc(const ConfigJuego& cfg, uint16_t vidaMaxNpc,
                                         float valorAleatorio) {
    if (vidaMaxNpc == 0) {
        return 0;
    }
...
uint16_t ReglasJuego::aplicarMultiplicadorCombate(uint16_t valor, float multiplicador) {
    ...
    return static_cast<uint16_t>(
            std::min<float>(static_cast<float>(std::numeric_limits<uint16_t>::max()), resultado));
}

float ReglasJuego::calcularRecuperacionMeditacion(const ConfigJuego& cfg, ClasePersonaje clase,
