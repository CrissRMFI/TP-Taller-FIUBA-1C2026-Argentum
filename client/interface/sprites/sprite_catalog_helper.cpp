...
const toml::table& SpriteCatalogHelper::require_subtable(const toml::table& table,
...
int SpriteCatalogHelper::require_int(const toml::table& table, const std::string& key,
                                     const std::string& path) {
...
uint16_t SpriteCatalogHelper::require_uint16_from_key(const toml::key& key,
                                                      const std::string& path) {
...
std::vector<uint16_t> SpriteCatalogHelper::require_uint16_array(const toml::table& table,
                                                                const std::string& key,
                                                                const std::string& path) {
    ...
    for (size_t index = 0; index < array->size(); ++index) {
        ...
        if (!value.has_value() || *value < 0 || *value > std::numeric_limits<uint16_t>::max()) {
...
SpriteVec2 SpriteCatalogHelper::optional_vec2_or_default(const toml::table& table,
                                                         const std::string& key,
...
std::optional<SpriteRect> SpriteCatalogHelper::optional_rect4(const toml::table& table,
                                                              const std::string& key,
                                                              const std::string& path) {
    if (!table.contains(key)) {
        return std::nullopt;
...
void SpriteCatalogHelper::validate_skin_parts_exist(
    ...
    for (const uint16_t head_id : preset.head_ids) {
        if (!heads.contains(head_id)) {
            throw_invalid(
                    join_path("skins", skin_name),
                    "head_id " + std::to_string(head_id) + " no existe en character_parts.heads");
        }
    }
    for (const uint16_t body_id : preset.body_ids) {
        if (!bodies.contains(body_id)) {
            throw_invalid(
                    join_path("skins", skin_name),
                    "body_id " + std::to_string(body_id) + " no existe en character_parts.bodies");
        }
    }
}

void SpriteCatalogHelper::validate_rows(const CharacterPartDefinition& part,
                                        const std::string& path) {
...
void SpriteCatalogHelper::validate_directions(const CharacterPartDefinition& part,
                                              const std::string& path) {
    for (int index = 0; index < 4; ++index) {
