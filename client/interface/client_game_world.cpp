...
void ObjectGameWorld::upload_server_msg(
        Queue<MensajeServidor>& server_msgs, const uint32_t current_tick,
    ...
    while (server_msgs.try_pop(mensaje)) {
        if (auto* entity_position = std::get_if<MensajePosicionEntidad>(&mensaje.payload)) {
            ...
            std::cout << "[cliente] esquive: entidad=" << esquive->idEntidad
                      << ", esquivador=" << static_cast<int>(esquive->esquivador) << std::endl;
    ...
    for (auto& [entity_id, animation_state] : animation_states) {
        animation_state.is_moving = (current_tick- animation_state.last_motion_tick) < MOTION_GRACE_MS;
    }
...
void ObjectGameWorld::notify_move_requested(const uint32_t tick_) {
    EntityAnimationState& animation_state = animation_states[idCliente];
    animation_state.is_moving = true;
    animation_state.last_motion_tick = tick_;
}
