...
void NPCRenderer::render(SDL2pp::Renderer& renderer, const Npc& npc, const int entity_x,
                         const int entity_y, const int cell_width, const int cell_height,
                         const int /*animation_row*/, const int /*frame_index*/,
    ...
    if (resolved.body.has_value()) {
        ...
        if (resaltar) {
            ...
            for (const auto& d :
                ...
                renderer.Copy(
                        *resolved.body->texture, to_sdl_rect(resolved.src),
