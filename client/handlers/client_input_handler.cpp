...
std::optional<ComandoJugador> ClientInputHandler::handle_keyboard(SDL_Keycode key) {
    switch (key) {
        ...
        case SDLK_F1:
            return ComandoJugador{Opcode::CHEAT,
...
ResultadoInput ClientInputHandler::handle_event(
    ...
    if (chat_activo) {
        if (event.type == SDL_MOUSEBUTTONDOWN && !click_en_chat(event.button.x, event.button.y)) {
            cerrar_chat();
...
std::optional<ComandoJugador> ClientInputHandler::handle_mouse_click(
    ...
    // El click siempre selecciona a la entidad (para los comandos de chat dirigidos a un objetivo:
    // comprar, curar, depositar, etc.).
...
uint16_t ClientInputHandler::buscar_id_objetivo(
    ...
    const int alto_juego =
            (window_height - chat_panel_alto > 0) ? window_height - chat_panel_alto : window_height;
    ...
    const float origenY =
            usarCamara ? static_cast<float>(cam_off_y) : static_cast<float>(chat_panel_alto);
...
bool ClientInputHandler::should_quit() const {
