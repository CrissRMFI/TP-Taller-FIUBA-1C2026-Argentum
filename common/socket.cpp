...
Socket::Socket(const char* hostname, const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         * */
        newSkt = socket(addr->ai_family, addr->ai_socktype, addr->ai_protocol);
        if (newSkt == -1) {
            continue;
        }
         ...
         * */
        s = connect(newSkt, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
...
Socket::Socket(const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         **/
        int optval = 1;
        s = setsockopt(newSkt, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
        if (s == -1) {
            continue;
        }
         ...
         * */
        s = bind(newSkt, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
